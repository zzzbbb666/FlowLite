# FlowLite - 轻量级可视化流程建模工具
FlowLite 是一款基于 C++ 与 Qt 框架开发的桌面端轻量级流程图建模软件。本项目旨在提供一个集图形渲染、交互操作、逻辑校验于一体的可视化编程教学平台。通过面向对象的设计思想，实现了从基础图元绘制、高阶交互、历史记录管理到数据持久化的完整闭环。

## 1. 开发环境配置
为确保项目可正常编译与运行，请配置以下环境：
  * **开发语言**: C++ 11 (或更高)
  * **开发框架**: Qt 5.14.2 (使用 Widgets, Gui, Core 模块)
  * **IDE**: Qt Creator
  * **编译器**: MinGW 64-bit (GCC)
  * **构建工具**: qmake

## 2.当前实现程度（核心功能）
## 阶段一：基础架构与画布搭建（100%）
![布局设计截图](./image/stage1.png)
![布局设计运行截图](./image/stage1-2.png)

## 阶段二：图形绘制与交互（100%）
![绘制截图](./image/stage2.png)

## 阶段三：连线与拓扑关系（100%）
![布局设计运行截图](./image/stage2-1.png)

## 阶段四：数据存储与文件操作（100%）
![数据存储运行截图](./image/stage3.png)
![数据读取运行截图](./image/stage3-1.png)

## 阶段五：逻辑校验与完善（100%）


1.删除功能
2.撤销/重做
3.视图
4.连线箭头
5.滚轮缩放
## 💻 开发环境与运行配置

为确保该项目的完美复现，请在以下配置环境中进行编译：

### 依赖项与环境
*   **开发语言**：C++ 11 或更高版本
*   **GUI 框架**：Qt 5.15+ 或 Qt 6.x（项目使用了 Core, Gui, Widgets 模块）
*   **推荐 IDE**：Qt Creator 10+ （或配置了 Qt 插件的 Visual Studio / CLion）
*   **编译器**：MSVC 2019+ (Windows) / MinGW 64-bit / GCC / Clang

### 运行环境搭建步骤
1. **克隆项目**：将本项目下载或 Clone 到本地目录，确保路径中无中文字符。
2. **加载工程**：打开 Qt Creator，点击 `文件` -> `打开文件或项目`，选择项目根目录下的 `FlowLite.pro` 文件。
3. **配置构建套件**：在弹出的项目配置页面，勾选你当前的 Desktop 构建套件（如 Desktop Qt 6.5.0 MinGW 64-bit），点击 `Configure Project`。
4. **编译与运行**：
    - 点击左下角的 🔨 图标（或按 `Ctrl+B`）进行项目构建。
    - 构建成功后，点击 ▶️ 图标（或按 `Ctrl+R`）运行项目。
  
## 📸 项目截图
![FlowLite 运行截图](./image/stage1.png)
